...
const SUBJECTS = [
  ...
  { 
    ...
    icon: "🔀", 
    color: "#F05032",
    thumbnailUrl: "https://images.unsplash.com/photo-1607799279861-4dd421887fb3?w=500&q=80" 
  },
];
...
async function seedLocal() {
  ...
  try {
    ...
    if (!contentSnap.empty) {
      ...
      await batch.commit();
      console.log(`Deleted ${contentSnap.size} old content items.`);
    }

    console.log("🌱 Seeding content items with beautiful thumbnails...");
    const now = Date.now();
    let seeded = 0;
    ...
    for (const subj of SUBJECTS) {
      ...
      const noteId = `notes-${subj.id}`;
      await db.collection("content").doc(noteId).set(noteDoc);
      seeded++;
      ...
      const pyqId = `pyq-${subj.id}`;
      await db.collection("content").doc(pyqId).set(pyqDoc);
      seeded++;
    }
    ...
    for (const pl of FREE_PLAYLISTS) {
      ...
      const videoId = `video-${pl.subjectId}`;
      await db.collection("content").doc(videoId).set(videoDoc);
      seeded++;
    }

    console.log(`✅ Successfully seeded ${seeded} content items with correct image links!`);
  } catch (error) {
